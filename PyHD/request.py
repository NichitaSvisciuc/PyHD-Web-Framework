...
class PostBody:
	...
	"""
	Data sent from forms are stored here
	"""
	__slots__ = 'data'
	...
	def get(self, key):
		...
		if type(value) == str:
			value = re.escape(value) # we escape values to prevent script injection attacks
		return value
...
class Request:
	...
	def __init__(self, environ, start_response):
		...
		self.META = {'HTTP_REFERER': environ.get('HTTP_REFERER')}
		self.POST = PostBody({})

		self.set_post_body()

	def set_post_body(self):
